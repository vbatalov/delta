...
public function getParamTvName($tv_id = '')
{
	...
	return mb_strtolower($this->modx->db->getValue("SELECT `name` FROM " . $this->modx->getFullTableName('site_tmplvars') . " WHERE id = {$tv_id} LIMIT 0,1"));
}
			...
			if (isset($filter_values_full[$tv_id])) {
				...
				switch ($filters[$tv_id]['type']) {
					...
					case '6': //слайдер-диапазон
						...
						if (100 == $tv_id) { //DELTA
							ksort($filter_values_full[$tv_id]);
							reset($filter_values_full[$tv_id]);
							$minvalcurr = key($filter_values_full[$tv_id]);
							end($filter_values_full[$tv_id]);
							$maxvalcurr = key($filter_values_full[$tv_id]);
						} else { //DELTA
...
public function getFilterValues ($content_ids, $filter_tv_ids = '')
{
	...
	if ($content_ids != '') {//берем только если есть какие-то документы
		...
		while ($row = $this->modx->db->getRow($q)) {
			if (101 == $row['tmplvarid']) {
				$row['tmplvarid'] = 100; //DELTA
			}
				...
				if (isset($v['min']) || isset($v['max'])) {//если параметр - диапазон
					...
					if (100 == $tvid) { //DELTA
						if (isset($v['min']) && (int)$v['min'] != 0 ) {
							$fltr .= $this->dl_filter_type . ':' . $this->filter_tv_names[101] . ':egt:' . (int)$v['max'] . ';';
						}
						if (isset($v['max']) && (int)$v['max'] != 0 ) {
							$fltr .= $this->dl_filter_type . ':' . $this->filter_tv_names[100] . ':elt:' . (int)$v['min'] . ';';
						}
