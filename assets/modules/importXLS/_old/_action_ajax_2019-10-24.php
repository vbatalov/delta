        ...
        foreach ($arrsXLS['data'] AS $indexString => $strCols) {
            ...
            if (is_array($ids)) {
                foreach ($ids AS $id) {
                    ...
                    $modx->db->query("DELETE FROM " . $modx->getFullTableName("site_content") . " WHERE id={$id} LIMIT 1");
                    $modx->db->query("DELETE FROM " . $modx->getFullTableName("site_tmplvar_contentvalues") . " WHERE contentid={$id}");
                }
            }
            ...
            if (!$finded && array_search("PGT", $tvCollation) !== false) {
                ...
                if ($strColsPGT == '' && $strCols[$flippedArrTv["CONTENT"]] == '') {
                    ...
                    $sql = "UPDATE  " . $modx->getFullTableName("site_content") . " SET pagetitle ='" . $modx->db->escape($strColsPGT) . "'  , deleted = 0, published = '1', parent = {$tocat} WHERE id = {$contentID} LIMIT 1";
                } else {
                    $sql = "UPDATE  " . $modx->getFullTableName("site_content") . " SET content ='" . $modx->db->escape($strCols[$flippedArrTv["CONTENT"]]) . "' ,   pagetitle='" . $modx->db->escape($strColsPGT) . "'  , deleted = 0, published = '1', parent = {$tocat} WHERE id = {$contentID} LIMIT 1";
                }
...
function processedTV($contentID, $flippedArrTv, $strCols, $modx, $vendorID) {
    foreach ($flippedArrTv AS $index => $value) {
        ...
        if (array_key_exists($value, $strCols)) {
            ...
            if ($index == '18') {
                $sql = "SELECT * FROM " . $modx->getFullTableName("site_tmplvar_contentvalues") . " WHERE tmplvarid = {$index} AND value={$strCols[$value]} LIMIT 1";
                if ($result = $modx->db->query($sql)) {
                    if ($modx->db->getRecordCount($result) > 0) {
                        $tmp = $modx->db->getRow($result);
                        $modx->db->query("DELETE FROM  " . $modx->getFullTableName("site_tmplvar_contentvalues") . " WHERE value={$strCols[$value]} AND tmplvarid=18");
                        $modx->db->query("INSERT INTO " . $modx->getFullTableName("site_tmplvar_contentvalues") . " (tmplvarid,contentid,value) VALUES (18 ,{$contentID} , '" . $strCols[$value] . "' )");
                        $strCols[$flippedArrTv["PGT"]] = str_replace('`', '', $strCols[$flippedArrTv["PGT"]]);
                        $modx->db->query("UPDATE " . $modx->getFullTableName("site_content") . " SET pagetitle=`{$strCols[$flippedArrTv["PGT"]]}` WHERE id={$contentID} ");
                    } else {
                        $modx->db->query("INSERT INTO " . $modx->getFullTableName("site_tmplvar_contentvalues") . " (tmplvarid,contentid,value) VALUES (18 ,{$contentID} , '" . $strCols[$value] . "' )");
                        $modx->db->query("UPDATE " . $modx->getFullTableName("site_content") . " SET pagetitle='{$strCols[$flippedArrTv["PGT"]]}' WHERE id={$contentID} ");
                    }
...
if (!empty($resources_not_empty)) {
    ...
    $modx->db->query("UPDATE " . $modx->getFullTableName("site_content") . " SET published = '1' WHERE id IN (" . $resources_not_empty . ") ");
}
...
if (!empty($catalog_not_empty)) {
    ...
    $modx->db->query("UPDATE " . $modx->getFullTableName("site_content") . " SET published = '1' WHERE id IN (" . $catalog_not_empty . ") ");
}
